...
from sinklint.rules import Config, Finding, evaluate
from sinklint.scanner import CtagsError, TypeDef, extract_typedefs, find_ctags, run_ctags
...
def test_find_ctags_nothing_on_path(monkeypatch):
    monkeypatch.setattr("sinklint.scanner.shutil.which", _fake_which({}))
...
def test_find_ctags_rejects_non_universal(monkeypatch):
    monkeypatch.setattr(
        "sinklint.scanner.shutil.which", _fake_which({"ctags": "/usr/bin/ctags"})
    )
    monkeypatch.setattr(
        "sinklint.scanner.subprocess.run",
...
def test_find_ctags_survives_broken_binary(monkeypatch):
    ...
    monkeypatch.setattr("sinklint.scanner.subprocess.run", boom)
    with pytest.raises(CtagsError, match="not found"):
        find_ctags()
